...
(CTAs) · `amber #DD8A38`.
**Secondary / situational:** `sage #8CA081` · `moss #2F3B2A` (footer) · `rose-soft #BC5E38`
(gentle alert). Green badge set for "no-anesthesia / safe" reserved for later.
**Line:** `line #EADEC2` hairline.

Strategy today: **Restrained** (tinted-neutral surfaces + gold accent under ~10%). The visual
pass is considering moving toward **Committed** in places (a real color moment per section /
per fold) so the page stops relying on the cream body to carry identity.

Contrast: enforce AA. Watch `brown`/`ink` on `honey-tint`/`sand`; muted body on tinted cream is
the palette's main contrast risk.
