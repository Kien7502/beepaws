...
// The homepage markup, with admin-authored blocks INJECTED rather than fetched
// here — so the live route (app/page.tsx) can pass the published metafield while
// the preview route passes the admin's unsaved draft. Same split as
// ProductPageView. Every slot falls back to the hardcoded content below when its
// block is absent, so `blocks={{}}` renders exactly today's page.
export function HomePageView({ blocks }: { blocks: HomepageBlocks }) {
  return (
