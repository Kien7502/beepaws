...
What this repo does instead: `createCart`/`createCartWithLines` stamp a `return_url` cart **attribute** (a real `CartInput` field) at `${NEXT_PUBLIC_SITE_URL}/thank-you` via `buildReturnAttributes()` in `lib/shopify/mutations.ts`. The `/thank-you` route (`components/checkout/ThankYouContent.tsx`) requires a `?cart_id=` param: it always records that id as completed (`markCartCompleted`, `CartProvider.tsx`) in a small localStorage ledger (`beepaws_completed_cart_ids`) that the hydration/restore path checks before ever re-presenting a cart, but only calls `clearCart()` when that id exactly matches the cart this browser currently has stored — Buy It Now builds a separate, unlinked cart per checkout, so a purchase completing elsewhere must not wipe an unrelated active drawer cart. Never add `completedAt` to the `CartFields` fragment; it's not a valid field on `Cart` and breaks every cart query.
